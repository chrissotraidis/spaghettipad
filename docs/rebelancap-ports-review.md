...
5. **License clarity — RESOLVED (2026-07-28): MIT.** His README carries a stale TODO ("pick a license for this repo's own code"), but the repo root contains a real MIT `LICENSE` (© 2026 rebelancap), verified by direct read. His shell and overlay patches are legally reusable with attribution — treat them as a reviewed starting point for adaptation, with provenance notes on anything adapted.
...
3. **His work makes yours easier and the claim window shorter.** The 47 patches are a public, current, MIT-licensed map of every LUS/SpaghettiKart iOS fix plus performance work (async Metal shaders, texture-cache eviction, supersampling) — reusable with attribution (§3.5), and equally available to anyone else who finds it.
