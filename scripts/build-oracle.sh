...
for command in cmake ninja curl; do
...
# Upstream configure always overwrites this build-directory input from master.
# Replace it with the previously accepted bytes before compiling; do not edit
# the unmodified oracle source or advance the expected checksum.
SSE2NEON_TEMP="$(mktemp "$BUILD_DIR/_deps/sse2neon/reviewed.XXXXXX")"
trap 'rm -f "$SSE2NEON_TEMP"' EXIT
curl --fail --location --silent --show-error \
    "https://raw.githubusercontent.com/DLTcollab/sse2neon/$SSE2NEON_PIN/sse2neon.h" \
    -o "$SSE2NEON_TEMP"
verify_download "$SSE2NEON_TEMP" "$EXPECTED_SSE2NEON_SHA256" "pinned sse2neon.h"
mv "$SSE2NEON_TEMP" "$BUILD_DIR/_deps/sse2neon/sse2neon.h"
